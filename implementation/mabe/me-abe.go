...
type MAABE struct {
	...
	egpk *bn256.GT
	egg  *bn256.GT
	APk  *bn256.G2 //the public key of the auditor
}
...
func (a *MAABE) NewMAABEAuth(id string, attribs []string) (*MAABEAuth, error) {
	...
	for _, at := range attribs {
		eggToAlpha[at] = new(bn256.GT).ScalarMult(a.egg, alpha[at])
...
func (auth *MAABEAuth) AddAttribute(attrib string) error {
	...
	y := skVals[1]
	// generate public key
	eggToAlpha := new(bn256.GT).ScalarMult(auth.Maabe.egg, alpha)
...
func (a *MAABE) Encrypt(msg string, msp *MSP, pks []*MAABEPubKey) (*MAABECipher, error) {
	...
	// fmt.Println("t1:", symKey.Marshal(), t2.Marshal(), t3.Marshal())
	if err != nil {
		return nil, err
	}
	...
	egps := new(bn256.GT).ScalarMult(a.egpk, s)
	c0 := new(bn256.GT).Add(symKey, egps)
	eggs := new(bn256.GT).ScalarMult(bn256.Pair(a.G1, a.G2), s)
	c0.Add(c0, eggs)
	c4 := new(bn256.G2).ScalarMult(a.G2, s)
	...
	for _, at := range msp.RowToAttrib {
		...
		for _, pk := range pks {
			if pk.EggToAlpha[at] != nil {
				...
				if signLambda >= 0 {
					tmpLambda = new(bn256.GT).ScalarMult(a.egpk, lambda[at])
				} else {
					tmpLambda = new(bn256.GT).ScalarMult(new(bn256.GT).Neg(a.egpk), new(big.Int).Abs(lambda[at]))
				}
				if signOmega >= 0 {
					tmpOmega = new(bn256.G2).ScalarMult(a.APk, omega[at])
				} else {
					tmpOmega = new(bn256.G2).ScalarMult(new(bn256.G2).Neg(a.APk), new(big.Int).Abs(omega[at]))
				}
...
func (a *MAABE) Decrypt(ct *MAABECipher, ks []*MAABEKey) (string, error) {
	...
	egps := new(bn256.GT).ScalarBaseMult(big.NewInt(0))
	for _, at := range goodAttribs {
		if eggLambda[at] != nil {
			sign := cx[at].Cmp(big.NewInt(0))
			if sign == 1 {
				egps.Add(egps, new(bn256.GT).ScalarMult(eggLambda[at], cx[at]))
			} else if sign == -1 {
				egps.Add(egps, new(bn256.GT).ScalarMult(new(bn256.GT).Neg(eggLambda[at]), new(big.Int).Abs(cx[at])))
			}
		} else {
	...
	eggs := bn256.Pair(a.G1, ct.C4)
	// calculate key for symmetric encryption
	symKey := new(bn256.GT).Add(new(bn256.GT).Add(ct.C0, new(bn256.GT).Neg(egps)), new(bn256.GT).Neg(eggs))
...
// Auditor represents a audit server who holds a tracing key to decrypt users' data
type Auditor struct {
	Maabe *MAABE
	SK    *big.Int
	PK    *bn256.G2
...
func (a *MAABE) InitAuditor(farce bool) (*Auditor, error) {
	if !farce && a.APk != nil {
		return nil, errors.New("the auditor has been initialized")
	}

	SK, err := sample.NewUniform(a.P).Sample()
	if err != nil {
		return nil, err
	}
	PK := new(bn256.G2).ScalarMult(a.G2, SK)

	a.APk = PK
	a.egpk = bn256.Pair(a.G1, a.APk)
	a.egg = bn256.Pair(a.G1, a.G2)
	return &Auditor{
		Maabe: a,
		SK:    SK,
		PK:    PK,
...
func (a *MAABE) Audit(ct *MAABECipher, tracingKey *big.Int) (string, error) {

	eggs := bn256.Pair(a.G1, ct.C4)
	egps := new(bn256.GT).ScalarMult(bn256.Pair(a.G1, ct.C4), tracingKey)
	// calculate key for symmetric encryption
	symKey := new(bn256.GT).Add(new(bn256.GT).Add(ct.C0, new(bn256.GT).Neg(egps)), new(bn256.GT).Neg(eggs))
	// now decrypt message with it
	keyCBC := sha256.Sum256([]byte(symKey.String()))
